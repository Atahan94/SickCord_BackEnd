let Server,getServerById,getUser,redisClient;_555‍.x([["setupSocketIO",()=>setupSocketIO],["default",()=>_555‍.o]]);_555‍.w("socket.io",[["Server",["Server"],function(v){Server=v}]]);_555‍.w("../controller/server",[["getServerById",["getServerById"],function(v){getServerById=v}]]);_555‍.w("../controller/user",[["getUser",["getUser"],function(v){getUser=v}]]);_555‍.w("../redis/redis-client",[["default",["redisClient"],function(v){redisClient=v}]]);
  ...
  io.on("connection", async (socket) => {
    ...
    _555‍.g.console.log("A user connected:", username);
    ...
    socket.on("serverMessage", async (data) => {
      try {
        _555‍.g.console.log("ServerMessage", data, user);
    ...
    socket.on("joinPrivateChat", async (data) => {
      try {
        ...
        _555‍.g.console.log("joinPrivateChat", data, "chat", chat, "user", user);
    ...
    socket.on("privateMessage", async (data) => {
      try {
        ...
        _555‍.g.console.log("privateMessage", data);
      } catch (error) {
        _555‍.g.console.error(error);
        socket.emit(
          "error",
    ...
    socket.on("JoinVoiceChannel", (data) => {
      room = data.id;
      socket.join(room);
      _555‍.g.console.log(username, "Joined voiceChannel with ID:", room);
    });
    ...
    socket.on("disconnectRoom", () => {
      socket.leave(room);
      _555‍.g.console.log(username, "Left voiceChannel with ID:", room);
    });

    socket.on("disconnect", () => {
      _555‍.g.console.log("A user disconnected:", username);
      redisClient.srem('onlineUsers', userName);
    });
  });
}

_555‍.d(setupSocketIO);
